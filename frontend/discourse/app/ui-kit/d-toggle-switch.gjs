    ...
    >
      ...
      {{#if @icon}}
        {{! Always rendered (unlike the text label), styled active/inactive
          ...
          tooltip convention. The hidden span gives the switch its accessible
          name, which the button references by id. }}
        <span class="sr-only" id={{this.labelId}}>{{this.computedLabel}}</span>
        <DTooltip
          class={{dConcatClass
